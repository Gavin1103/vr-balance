...
import vr.balance.app.API.response.ExerciseStatsResponse;
...
public interface CompletedExerciseRepository extends JpaRepository<CompletedExercise, Long> {
     ...
     */
    @Query("""
                SELECT new vr.balance.app.API.response.ExerciseStatsResponse(
                    ce.exercise,
                    COUNT(ce)
                )
    ...
    @Query("""
                ...
                GROUP BY ce.exercise, u.username
                ORDER BY SUM(ce.earnedPoints) DESC
            """)
    List<Object[]> findTopByExercise(@Param("exercise") ExerciseEnum exercise, Pageable pageable);
