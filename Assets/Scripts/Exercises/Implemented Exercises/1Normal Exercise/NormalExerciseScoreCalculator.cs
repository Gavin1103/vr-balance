...
public class NormalExerciseScoreCalculator {
    ...
    public NormalExerciseScoreCalculator() {
    }
    ...
    public void CalculateDistances(Vector3 leftTarget, Vector3 rightTarget, Vector3 headTarget)
    {
        ...
        NormalExerciseReferences.Instance.LeftTextPercentage.text = LeftPercentage <= 0f ? "L TOO FAR!" : $"L {LeftPercentage:F0}%";
        NormalExerciseReferences.Instance.RightTextPercentage.text = RightPercentage <= 0f ? "R TOO FAR!" : $"R {RightPercentage:F0}%";
        NormalExerciseReferences.Instance.HeadTextPercentage.text = HeadPercentage <= 0f ? "H TOO FAR!" : $"H {HeadPercentage:F0}%";

        // Update Affordance Pulses
        NormalExerciseReferences.Instance.LeftPulseAffordance.SetPulseScale(HeadPercentage);
        NormalExerciseReferences.Instance.RightPulseAffordance.SetPulseScale(HeadPercentage);
        NormalExerciseReferences.Instance.HeadsetPulseAffordance.SetPulseScale(HeadPercentage);
    }
    ...
    public void DecideFeedback(float score, float maxScore) {
        Vector3 popupSpawnPosition = NormalExerciseReferences.Instance.MovementImageObject.transform.position + new Vector3(0, 0.5f, 0);
